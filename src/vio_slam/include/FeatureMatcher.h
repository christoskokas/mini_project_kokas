...
class FeatureMatcher
{
    private:
        ...
        void matchKeys(const std::vector < cv::KeyPoint >& leftKeys, const std::vector<std::vector < int > >& indexes, const cv::Mat& leftDesc, const cv::Mat& rightDesc, std::vector <cv::DMatch>& matches);
        int DescriptorDistance(const cv::Mat &a, const cv::Mat &b);
        
    public:
        FeatureMatcher(const int _imageHeight = 360, const int _stereoYSpan = 3);
